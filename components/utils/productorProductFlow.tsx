...
export default function ProductFlow({ mode, initialData }: ProductFlowProps) {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm<ProductFormData>({
    defaultValues: {
      ...
      quantity: initialData?.quantityForSale?.toString() || initialData?.quantity?.toString() || '',
    }
  });
  ...
  // File & Audio State (Managed separately for custom UI)
  ...
  const onSubmit: SubmitHandler<ProductFormData> = async (formData) => {
    ...
    // Vérification critique du userId
    if (!user?.id) {
        setErrorMsg("Erreur d'identification. Veuillez vous reconnecter.");
        return;
    }
    ...
    try {
      ...
      // Utilisation de userId comme attendu par l'API
      data.append('userId', user.id); 
      data.append('mode', mode);
      if (mode === 'edit') data.append('id', initialData.id);
    ...
    <div className="min-h-screen bg-slate-50 pb-20 font-sans relative">
      ...
      <div className="bg-white p-4 sticky top-0 z-30 shadow-sm border-b border-slate-100">
        <div className="flex items-center gap-4 mb-4">
          <button onClick={() => step > 1 ? setStep(step - 1) : router.back()} className="p-2 bg-slate-100 rounded-full">
            ...
            <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
              ...
              <div className="flex gap-4">
                ...
                <select 
                  ...
                  {...register('unit')}
                >
                  <option>KG</option><option>SAC</option><option>TONNE</option><option>UNITÉ</option>
                </select>
              </div>
            </div>
            <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
